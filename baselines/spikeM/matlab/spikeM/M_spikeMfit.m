...
function [RSE, params] = M_spikeMfit( T, dat, pfreq, outfn, iter, wantPlot )
    %% fitting SpikeM
    datOld = dat;
    dat = dat(1:T);
    [RSE, params] = LMFit(dat, pfreq, iter, wantPlot);
    [n,b,u]=M_spikeM(length(datOld), params);
    save([outfn,'.b'], 'b', '-ascii');
    save([outfn,'.u'], 'u', '-ascii');
    %% plot fitting results 
    %plotsRNF(datOld, params, outfn);
